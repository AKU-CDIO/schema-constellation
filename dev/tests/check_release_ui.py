...
with sync_playwright() as p:
    ...
    assert page.locator(".topic-block").count() == 30
    assert page.locator("[data-proc]").count() == 31
    assert page.locator("#spN").inner_text() == "30"
    assert page.locator("#spImplemented").inner_text() == "26"
    assert page.locator("#spAssets").inner_text() == "31"
    ...
    assert "BLUEPRINT" in page.locator("#topic-surgery").inner_text()

    page.goto(BASE + "explorer.html")
    page.wait_for_timeout(1700)
    assert page.locator(".node").count() == 170
    assert page.locator(".edge").count() == 101
